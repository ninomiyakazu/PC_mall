...
class JDBCExample2 {
    ...
    //1단계:DB 연결을 위한 커넥션 인터페이스  
       ...
       try {
          ...
          rs= stmt.executeQuery("select jumincd, pname, gender, age from person where gender ");
          
           System.out.println("주민번호  이름   성별  나이 ");
          
          while( rs.next()) {
           
           jumicd = rs.getString(1); //rs.getString("jumicd");
            pname = rs.getString(2); //rs.getString("pname");
            gender = rs.getString(3); //rs.getString("gender");
           age        = rs.getInt(4);  // rs.getString("age");
           
           System.out.println(jumicd + "----" +pname + "----" + gender + "----"+ age);
          }
          //4단계 : DB연결을 종료한다. 
           conn.close();       
           }
